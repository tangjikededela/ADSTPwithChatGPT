...
# which aim to figure out what kinds of clients subscribed to a term deposit?
bank_dataset = read_csv("./data/banking.csv", header=0)
# Step 2: Convert some variable columns that contain only finite elements into multiple columns that contain only 0 or 1.
...
bank_dataset = read_csv("./data/banking.csv", header=0)
data_final = IP.create_dummy_variables(bank_dataset, cat_vars)
...
# Step 4: Choose the model, the independent and the dependent variables,
# then, replace the independent and dependent variables, and set questions.
pipeline = IP.general_datastory_pipeline
